...
rm -rf $test_dir
mkdir -p $test_dir
ssh $XDDTEST_E2E_DEST "rm -rf $XDDTEST_DEST_MOUNT/xddcp-multinic-r-a-n"
rm .xdd*
...
#        fi
	...
	$XDDTEST_XDDCP_EXE $xddcp_opts -r -a -n 99 $test_dir $DEST_DATA_NETWORK_1:$DEST_DATA_NETWORK_2:$DEST_DATA_NETWORK_3:$XDDTEST_DEST_MOUNT
        rc=$?
done
	...
	if [ "${srcHash[$k]}" != "$destHash" ]; then
	    test_passes=0
	    echo "ERROR: Failure in xddcp-multinic-r-a-n"
...
if [ "1" == "$test_passes" ]; then
  echo "Acceptance XDDCP: Multinic Recursive Restart Test - Check: PASSED."
  exit 0
else
  echo "Acceptance XDDCP: Multinic Recursive Restart Test - Check: FAILED."
  exit 1
fi
