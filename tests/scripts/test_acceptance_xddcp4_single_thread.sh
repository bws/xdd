...
#
$XDDTEST_XDDCP_EXE $xddcp_opts -t 1 $test_file $XDDTEST_E2E_DEST:$XDDTEST_DEST_MOUNT/v1-1
rc=$?
if [ 0 -ne $rc ]; then
    echo "Failure: transfer failed: $XDDTEST_XDDCP_EXE $xddcp_opts -t 1 $test_file $XDDTEST_E2E_DEST:$XDDTEST_DEST_MOUNT/v1-1"
    test_passes=0
else
...
$XDDTEST_XDDCP_EXE $xddcp_opts -t 1 -v $test_file $XDDTEST_E2E_DEST:$XDDTEST_DEST_MOUNT/v1-2
rc=$?
if [ 0 -ne $rc ]; then
    echo "Failure: transfer failed: $XDDTEST_XDDCP_EXE $xddcp_opts -t 1 -v $test_file $XDDTEST_E2E_DEST:$XDDTEST_DEST_MOUNT/v1-2"
    test_passes=0
else
...
$XDDTEST_XDDCP_EXE $xddcp_opts -t 1 -V $test_file $XDDTEST_E2E_DEST:$XDDTEST_DEST_MOUNT/v1-3
rc=$?
if [ 0 -ne $rc ]; then
    echo "Failure: transfer failed: $XDDTEST_XDDCP_EXE $xddcp_opts -t 1 -V $test_file $XDDTEST_E2E_DEST:$XDDTEST_DEST_MOUNT/v1-3"
    test_passes=0
else
