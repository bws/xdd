...
function handle_exit
{
    ...
    pkill -u ${XDDTEST_USER} -P 1 -x xdd >/dev/null 2>&1
    sleep 1
    pkill -KILL -u ${XDDTEST_USER} -P 1 -x xdd >/dev/null 2>&1

    # Kill any outbound SSH processes that have been left open
    pkill -u ${XDDTEST_USER} -x ssh >/dev/null 2>&1
    sleep 1
    pkill -KILL -u ${XDDTEST_USER} -x ssh >/dev/null 2>&1
...
{
    ps -aef |grep ${XDDTEST_USER} > before_timeout_killer
    g_testTimedOut=0
    if [ 0 -ne $g_testPID ]; then
        ...
        pkill -u ${XDDTEST_USER} -P 1 xdd >/dev/null 2>&1
        sleep 1
        pkill -KILL -u ${XDDTEST_USER} -P 1 xdd >/dev/null 2>&1
    ...
    ps -aef |grep ${XDDTEST_USER} > after_timeout_killer
...
for test in $all_tests; do
    ...
    ps -aef |grep ${XDDTEST_USER} >> $test_log
    echo "Process Table before Test $test_base -------------" >> $test_log
