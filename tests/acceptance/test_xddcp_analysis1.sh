...
\which iotrace_init
if [ 0 -ne $? ]; then
  echo "Acceptance XDDCP-w: XDDCP Post Analysis w/o Kernel Tracing - iotrace_init missing...test: SKIPPED."
  exit 1
fi
\which decode
if [ 0 -ne $? ]; then
  echo "Acceptance XDDCP-w: XDDCP Post Analysis w/o Kernel Tracing - decode missing...test: SKIPPED."
  exit 1
fi
if [ ! -e /dev/iotrace_data ]; then
  echo "Acceptance XDDCP-w: XDDCP Post Analysis w/o Kernel Tracing - /dev/iotrace_data missing...test: SKIPPED."
  exit 1
fi
