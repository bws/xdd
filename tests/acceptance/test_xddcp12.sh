...
src_dir=$XDDTEST_SOURCE_MOUNT/xddcp_test12/data
dest_dir=$XDDTEST_DEST_MOUNT/xddcp_test12/data
rm -rf $XDDTEST_SOURCE_MOUNT/xddcp_test12
ssh -q $XDDTEST_E2E_DEST "rm -rf $XDDTEST_DEST_MOUNT/xddcp_test12"
...
$XDDTEST_XDD_EXE -targets ${#targets[@]} ${targets[@]:0} -op write -reqsize 4096 -mbytes 2048 -qd 4 -datapattern randbytarget 

# Build file list
cd ${src_dir}
file_list="$XDDTEST_SOURCE_MOUNT/xddcp_test12/xddcp_test12_flist"
...
# Kill just the remote side using "-op write" to match the remote xdd
for ((n=0; n < 10; n += 1)); do
    ssh -q $XDDTEST_E2E_DEST "pkill -f \"\\-op write\""
    sleep 20
done
EOF
) &
...
if [ "1" == "$test_passes" ]; then
  echo "Acceptance XDDCP: Test 12 - Check: PASSED."
  exit 0
else
  echo "Acceptance XDDCP: Test 12 - Check: FAILED."
  exit 1
fi
