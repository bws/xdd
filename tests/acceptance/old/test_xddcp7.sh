...
exit_success(){
  echo "Acceptance: Recursive Test xddcp_single_thread - Check: PASSED."
  exit 0
}

exit_error(){
  echo "Acceptance: Recursive Test xddcp_single_thread - Check: FAILED."
  exit 1
}
...
test_dir=$XDDTEST_SOURCE_MOUNT/xddcp_single_thread
rm -rf $test_dir
mkdir -p $test_dir || exit_error
rm -rf $XDDTEST_DEST_MOUNT/xddcp_single_thread
...
if [ 0 -eq $rc ]; then
	...
	if [ "$srcHash" != "$destHash" ]; then
	    test_passes=0
	    echo "ERROR: Failure in xddcp_single_thread"
