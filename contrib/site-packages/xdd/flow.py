...
class XDDFlowStatus(FlowStatus):
    ...
    def output(self, flushStderr=False):
        ...
        # Retrieve any data grabbed from stderr for current byte progress
        if flushStderr:
            self.currentByte()
        data = self.errorData
...
class Flow():
    ...
    def __init__(self, isSink, reqSize, flowIdx, numFlows, ifaces,
        ...
        if '' != xddPath:
            ...
            # Try to use 'type -p' to find xdd's path
            cmd = ['type', '-p', 'xdd']
    ...
    def getRestartFilename(self, filename):
        """@return the name of the restart file for this sink"""
        # The restart file is named .sinkfile-idx-n.xrf
    ...
    def getRestartOffset(self, filename):
        ...
        offset = 0
        try:
            rfn = self.getRestartFilename(filename)
            rf = open(rfn, 'r')
            cookie = rf.readline();
            rf.close()
        except:
            print('Unable to open file', rfn)
        try:
            cl = cookie.split(' ')
            offset = int(cl[2])
        except:
            print('Invalid restart cookie found', cookie)
        return offset
    ...
    def start(self, filename, dataSize, restartFlag=False, restartOffset=0):
        ...
        if self.checkPreconditions():
            # Use the file creator for size 0 flags, XDD for everything else
            if 0 == dataSize:
                isCreator = (0 == self.flowIdx) and isSink
