            ...
            for e in endpoints:
                ...
                if e.completionStatus() is not None:
                    ...
                    if 0 != e.completionStatus():
                        failureCount += 1
                # Get the progress (only sources should report non-zero)
                currentBytes = e.currentByte()
                if 0 < currentBytes:
                    completedBytes += currentBytes
            
            # Print a status message
            print('Progress', completedBytes, self.currentRestartOffset)
            if 0 >= completedBytes - self.currentRestartOffset:
                self.showProgress(self.currentSourceFile, 0)
            else:
                self.showProgress(self.currentSourceFile, 
                                  completedBytes - self.currentRestartOffset)
            self.writeLog()
            monEnd = time.time()
