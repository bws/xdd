...
class TransferManager:
    ...
    def startFlows(self, sourceFile, sinkFile):
        ...
        restartOffset = 0
        if self.restartFlag:
            s = self.factory.getSinkFlowBuilders()[0]
    ...
    def monitorFlows(self, monitorInterval):
        ...
        while completeCount < len(builders):
            ...
            for b in builders:
                ...
                if (completedByte == 0) or (byte < completedByte):
                    completedByte = byte
            self.showProgress(self.currentSourceFile, completedByte)
            self.writeLog()
            monEnd = time.time()
        ...
        failures = 0
        for b in builders:
            if 0 != b.completionStatus():
                failures += 1
                print('RC:', b.completionStatus())
                print('ERROR:', b.errorString())
            
        # If the completion status for all builder is success, print out
        # a final progress message and remove 
        if 0 == failures:
    ...
    def startAndMonitorFlows(self, monitorInterval):
        ...
        if 0 == rc:
            for (sfile, tfile) in files:
                if self.restartFlag and sink.fileIsComplete(self.sinkTarget, 
                                                            tfile):
                ...
                if 0 != rc:
                    ...
                    break
                rc = self.monitorFlows(monitorInterval)
                if 0 != rc:
                    print('ERROR transferfing', sfile)
                    break
                if self.restartFlag and 0 == rc:
                    sink.markFileCompleted(self.sinkTarget, tfile)
