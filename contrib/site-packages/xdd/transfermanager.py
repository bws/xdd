...
class TransferManager:
    ...
    def monitorTransfer(self, monitorInterval):
        ...
        builders = self.factory.getEndpoints()
        completedByte = 0
        completeCount = 0
        failureCount = 0
        while completeCount < len(builders) and 0 == failureCount:
            monBegin = time.time()
            completedByte = 0
            completeCount = 0
            for b in builders:
        ...
        failures = 0
        for b in builders:
            if b.completionStatus() is None:
                b.cancelFlow()
            elif 0 != b.completionStatus():
                failures += 1
    ...
    def cleanupEndpoints(self):
        ...
        rc = 0
        if self.factory:
            flowBuilders = self.factory.getEndpoints()
