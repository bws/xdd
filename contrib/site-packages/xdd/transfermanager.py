...
class TransferManager:
    ...
    def startFlows(self, sourceFile, sinkFile):
        ...
        for s in sinks:
            r = s.startFlow(sinkFile, self.transferSize, self.restartFlag, 0)
            if 0 != r:
        ...
        for s in sources:
            r = s.startFlow(sourceFile, self.transferSize, self.restartFlag, 0)
            if 0 != r:
    ...
    def startAndMonitorFlows(self, monitorInterval):
        ...
        """
        # Get the list of source files from the first source if its a directory
        ...
        if 0 == rc:
            ...
            for d in dirs:
                sink.buildDir(self.sinkTarget + '/' + d)

            # Transfer all files
            if files:
                for f in files:
                    rc = self.startFlows(f, self.sinkTarget + '/' + f)
                    if 0 != rc:
                        break
                    self.monitorFlows(monitorInterval)
            else:
                rc = self.startFlows(self.sourceTarget, self.sinkTarget)
                if 0 == rc:
                    self.monitorFlows(monitorInterval)
            if 0 != rc:
                # TODO: This should raise an error I guess
                pass

            # Create all symlinks
            for l in links:
                (target, linkname) = l
                sink.buildSymlink(self.sinkTarget + '/' + target,
                                  self.sinkTarget + '/' + linkname)
        return rc
    ...
    def showStatus(self, completedBytes):
        ...
        padding = ' '
        if cols > (len(status) + len(self.sourceTarget)):
            padding *= cols - len(status) - len(self.sourceTarget)
        
        # Clear the existing status line with whitespace
        print('\r', ' ' * cols, sep='', end='', file=sys.stderr)
         
        # Write the new status line
        print('\r', self.sourceTarget, padding, status, 
