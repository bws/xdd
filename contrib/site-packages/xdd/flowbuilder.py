...
class FlowBuilder:
    ...
    def createDirectory(self, target, mode=0644):
        """
    ...
    def createSymlink(self, source, target):
        ...
        try:
            ...
            # If the create failed, check and see if it exists
            if os.path.islink(source) and target == os.readlink(source):
                rc = 0
            else:
                rc = 1
        return rc
    ...
    def errorString(self):
        return self.currentFlowStatus.getErrorString()

    def output(self):
        """@return the flow's standard output"""
        return self.currentFlowStatus.getOutput()
