...
def createTransferManager(src, dest, opts):
    ...
    if not sourceTuples:
        ...
        for s in sourceTuples:
            (user, host) = s
            transferMgr.addSource(user, host, opts.threads, 
                                  [destHost], opts.port)
...
def performTransfer(transferMgr, retryLimit):
    ...
    while count < retryLimit:
        ...
        try:
            ...
            if 0 != rc:
                raise CreateFlowTransferError()

             # Start and monitor the flows
            rc = transferMgr.startAndMonitorFlows(1)
            if 0 != rc:
                raise StartFlowTransferError()
            else:
                break
