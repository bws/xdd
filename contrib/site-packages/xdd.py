...
class FlowBuilderTransportForwardingServer(SocketServer.TCPServer):
    ...
    def setTransport(self, transport, remoteHost, remotePort):
...
class FlowBuilderTransportForwardingHandler(SocketServer.BaseRequestHandler):
    ...
    def handle(self):
        ...
        try:
            destAddr = (self.server.remoteHost, self.server.remotePort)
            srcAddr = self.server.server_address
            channel = self.server.transport.open_channel('forwarded-tcpip',
                                                         destAddr, srcAddr)
...
class FlowBuilderTransport:
    ...
    def shutdown(self):
        ...
        the ssh connection
        """
        print("Testing Proxy", self.flowBuilder.test())
        #try:
    ...
    def createRemoteServer(self, host):
        ...
        try:
            ...
            print('SSH Exception', host)

        # Return the SSH session and URI
        return (ssh, uri)

    def startLocalForwarder(self, remotePort):
        """
        ...
        forwardServer = FlowBuilderTransportForwardingServer(
            ('', 0), FlowBuilderTransportForwardingHandler)
        forwardServer.setTransport(self.ssh.get_transport(), 
                                   self.host, remotePort)
        ...
        # Try to perform a quick lookup
        fields = self.remoteURI.rsplit(':', 1)
        uri = fields[0] + ':' + str(port)
        print("URI:", uri)
        forwardThread.join()
        f = Pyro4.Proxy(uri)
        print(f.testString())
