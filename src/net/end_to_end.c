...
xdd_e2e_src_send(worker_data_t *wdp) {
	...
	wdp->wd_e2ep->e2e_header.sequence = wdp->wd_task.task_op_number;
	wdp->wd_e2ep->e2e_header.location = wdp->wd_task.task_byte_offset;
	...
	// Note: the wdp->wd_e2ep->e2e_xfer_size is the size of the data field plus the size of the header
	maxmit = MAXMIT_TCP;
	sent = 0;
        sentcalls = 0;
...
fprintf(stderr,"E2E_SOURCE_SIDE_SEND: Sent %d bytes\n",wdp->wd_e2ep->e2e_send_status);
	...
	if (tdp->td_tsp->ts_options & (TS_ON | TS_TRIGGERED)) {
		tdp->td_ttp->tte[wdp->wd_ts_entry].net_xfer_size = wdp->wd_e2ep->e2e_xfer_size;
		...
		tdp->td_ttp->tte[wdp->wd_ts_entry].net_xfer_calls = sentcalls;
	}
	
	// Calculate the Send/Receive time by the time it took the last sendto() to run
	wdp->wd_e2ep->e2e_sr_time = (wdp->wd_counters.tc_current_net_end_time - wdp->wd_counters.tc_current_net_start_time);

	if (sent != wdp->wd_e2ep->e2e_xfer_size) {
...
fprintf(stderr,"E2E_DEST_SIDE_RECEIVE: RECEIVED an E2E_MAGIQ\n");
			...
			if (!wdp->wd_e2ep->e2e_wait_1st_msg) 
				wdp->wd_e2ep->e2e_wait_1st_msg = wdp->wd_counters.tc_current_net_end_time - e2e_wait_1st_msg_start_time;
			...
			if ((wdp->wd_e2ep->e2e_header.magic != PTDS_E2E_MAGIC) &&  
   			...
   			if ((tdp->td_tsp->ts_options & (TS_ON|TS_TRIGGERED))) {
				tdp->td_ttp->tte[wdp->wd_ts_entry].net_xfer_size = wdp->wd_e2ep->e2e_recv_status;
...
fprintf(stderr,"E2E_EOF_SOURCE_SIDE: Sent %d bytes\n",wdp->wd_e2ep->e2e_send_status);
	...
	// If time stamping is on then we need to reset these values
   	if ((tdp->td_tsp->ts_options & (TS_ON|TS_TRIGGERED))) {
		...
		tdp->td_ttp->tte[wdp->wd_ts_entry].net_end = wdp->wd_counters.tc_current_net_end_time;
		tdp->td_ttp->tte[wdp->wd_ts_entry].net_processor_end = xdd_get_processor();
		tdp->td_ttp->tte[wdp->wd_ts_entry].net_xfer_size = sent;
		tdp->td_ttp->tte[wdp->wd_ts_entry].net_xfer_calls = sentcalls;
