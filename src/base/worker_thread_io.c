...
xdd_worker_thread_io(worker_data_t *wdp) {
	...
	target_data_t		*tdp;				// Pointer to the Target PTDS for this Worker Thread
...
fprintf(xgp->errout,"\n%s: xdd_worker_thread_io: Target %d Worker Thread %d: ENTER: tdp=%p, wdp=%p\n", xgp->progname, tdp->td_target_number, wdp->wd_thread_number,tdp,wdp);
	...
	if ((xgp->canceled)  || (xgp->abort) || (tdp->td_tgtstp->abort) || (tdp->td_tgtstp->abort)) {
	...
	tdp->td_tgtstp->my_current_state |= CURRENT_STATE_IO;
	xdd_io_for_os(wdp);
	tdp->td_tgtstp->my_current_state &= ~CURRENT_STATE_IO;

	// Update counters and status in this Worker Thread's PTDS
...
xdd_worker_thread_wait_for_previous_io(worker_data_t *wdp) {
	...
	tot_offset = (tdp->td_tgtstp->target_op_number % tdp->td_totp->tot_entries) - 1;
	if (tot_offset < 0) 
	...
	if (tdp->td_target_options & TO_E2E_DESTINATION) {
		...
		if (tdp->td_tgtstp->target_op_number == 0)
			return(0);	// Dont need to wait for op minus 1 ;)
	}


	tep = &tdp->td_totp->tot_entry[tot_offset];
	tdp->td_tgtstp->my_current_state |= CURRENT_STATE_WT_WAITING_FOR_TOT_LOCK_TS;
	...
	tdp->td_tgtstp->my_current_state |= CURRENT_STATE_WT_WAITING_FOR_PREVIOUS_IO;
...
xdd_worker_thread_release_next_io(worker_data_t *wdp) {
	...
	tdp = wdp->wd_tdp;
	tot_offset = (tdp->td_tgtstp->target_op_number % tdp->td_totp->tot_entries);

	// Wait for the I/O operation ahead of this one to complete (if necessary)

	tep = &tdp->td_totp->tot_entry[tot_offset];
	tdp->td_tgtstp->my_current_state |= CURRENT_STATE_WT_WAITING_FOR_TOT_LOCK_RELEASE;
	pthread_mutex_lock(&tep->tot_mutex);
	tdp->td_tgtstp->my_current_state &= ~CURRENT_STATE_WT_WAITING_FOR_TOT_LOCK_RELEASE;
 ...
 * PTDS counters and timers in this Worker Thread's PTDS
 *
 * The worker_thread_io_for_os() subroutine issues an I/O operation and sets the
 * "my_current_io_status" member of this Worker Thread's PTDS equal to whatever the
 ...
 * Worker Thread's PTDS are updated appropriately. 
...
xdd_worker_thread_update_local_counters(worker_data_t *wdp) {
	target_data_t	*tdp;			// Pointer to the Tartget's PTDS
...
//			xgp->abort = 1; // This tells all the other Targets to abort
	...
	tdp->td_tgtstp->my_current_error_count = 0;
	if (tdp->td_tgtstp->my_current_io_status == tdp->td_tgtstp->my_current_io_size) { // Status is GOOD - update counters
		tdp->td_tgtstp->my_current_bytes_xfered_this_op = tdp->td_tgtstp->my_current_io_size;
		tdp->td_tgtstp->my_current_bytes_xfered += tdp->td_tgtstp->my_current_io_size;
		tdp->td_tgtstp->my_current_op_count++;
		// Operation-specific counters
		switch (tdp->td_tgtstp->my_current_op_type) { 
			case OP_TYPE_READ: 
				tdp->td_tgtstp->my_accumulated_read_op_time += tdp->td_tgtstp->my_current_op_elapsed_time;
				tdp->td_tgtstp->my_current_bytes_read += tdp->td_tgtstp->my_current_io_size;
				tdp->td_tgtstp->my_current_read_op_count++;
...
//			xgp->abort = 1; // This tells all the other Targets to abort
		...
		fprintf(xgp->errout, "%s: I/O ERROR on Target %d Worker Thread %d: ERROR: Status %d, I/O Transfer Size [expected status] %d, %s Operation Number %lld\n",
			...
			tdp->td_tgtstp->my_current_io_size,
			tdp->td_tgtstp->my_current_op_str,
			(long long)tdp->td_tgtstp->target_op_number);
		if (!(tdp->td_target_options & TO_SGIO)) {
			if ((tdp->td_tgtstp->my_current_io_status == 0) && (errno == 0)) { // Indicate this is an end-of-file condition
				fprintf(xgp->errout, "%s: Target %d Worker Thread %d: WARNING: END-OF-FILE Reached during %s Operation Number %lld\n",
					xgp->progname,
					tdp->td_target_number,
					wdp->wd_thread_number,
 ...
 * NOTE: This routine locks the Target PTDS so that no other Worker Threads can be
...
xdd_worker_thread_update_target_counters(worker_data_t *wdp) {
	...
	pthread_mutex_lock(&tdp->td_counter_mutex);
	// Update counters and status in the Worker Thread PTDS
	tdp->td_tgtstp->my_accumulated_op_time += tdp->td_tgtstp->my_current_op_elapsed_time;
	if (tdp->td_tgtstp->my_current_io_status == tdp->td_tgtstp->my_current_io_size) { // Only update counters if I/O succeeded
		tdp->td_tgtstp->my_current_bytes_xfered_this_op = tdp->td_tgtstp->my_current_bytes_xfered_this_op;
		tdp->td_bytes_completed += tdp->td_tgtstp->my_current_bytes_xfered_this_op;
		tdp->td_tgtstp->my_current_bytes_xfered += tdp->td_tgtstp->my_current_bytes_xfered_this_op;
		tdp->td_tgtstp->my_current_op_count++;
		...
		if ( (tdp->td_target_options & TO_ENDTOEND) && (tdp->td_target_options & TO_E2E_DESTINATION)) {
			...
			if ( tdp->td_tgtstp->my_pass_start_time < tdp->td_tgtstp->my_pass_start_time)  // Record the proper PASS start time
				tdp->td_tgtstp->my_pass_start_time = tdp->td_tgtstp->my_pass_start_time;
		}
		// Operation-specific counters
		switch (tdp->td_tgtstp->my_current_op_type) { 
			case OP_TYPE_READ: 
	...
	tdp->td_tgtstp->my_current_error_count += tdp->td_tgtstp->my_current_error_count;

	pthread_mutex_unlock(&tdp->td_counter_mutex);
	...
	if (tdp->td_tgtstp->my_current_error_count) 
		return; 
	...
	if (tdp->td_target_options & (TO_ORDERING_STORAGE_SERIAL | TO_ORDERING_STORAGE_LOOSE)) {
	    tdp->td_tgtstp->my_current_state |= CURRENT_STATE_WT_WAITING_FOR_TOT_LOCK_UPDATE;
	    tot_update(tdp->td_totp,
		       tdp->td_tgtstp->target_op_number,
		       wdp->wd_thread_number,
		       tdp->td_tgtstp->my_current_byte_location,
		       tdp->td_tgtstp->my_current_io_size);
	    tdp->td_tgtstp->my_current_state &= ~CURRENT_STATE_WT_WAITING_FOR_TOT_LOCK_UPDATE;
	}
