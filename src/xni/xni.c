...
int xni_accept_connection(xni_context_t ctx, struct xni_endpoint* local, int nbuf, size_t bufsiz, xni_connection_t* conn)
{
  return ctx->protocol->accept_connection(ctx, local, nbuf, bufsiz, conn);
}

//TODO: local_endpoint
int xni_connect(xni_context_t ctx, struct xni_endpoint* remote, int nbuf, size_t bufsiz, xni_connection_t* conn)
{
  return ctx->protocol->connect(ctx, remote, nbuf, bufsiz, conn);
}
...
{
  return (*buffer)->connection->context->protocol->send_target_buffer(buffer);
}
