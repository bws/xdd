...
/*----------------------------------------------------------------------------*/
// No Ordering - Turn off Loose and Strict ordering of QThread I/O
// aka -no 
int
xddfunc_noordering(int32_t argc, char *argv[], uint32_t flags)
{
    ...
    // At this point the "target_number" is valid
	if (target_number >= 0) { 
		/* Unset the Loose and Strict Ordering Opetions for a specific target */
		p = xdd_get_ptdsp(target_number, argv[0]);
		if (p == NULL) return(-1);
        ...
        return(args+1);
			...
			/* Unset the Loose and Strict Ordering Opetions for all targets */
			if (flags & XDD_PARSE_PHASE2) {
				p = xgp->ptdsp[0];
				i = 0;
				while (p) {
