...
struct xint_worker_data {
	...
	xdd_timestamp_t				*wd_tsp;   			// Pointers to the timestamp table 
	xdd_tthdr_t					*wd_ttp;   			// Pointers to the timestamp table header
	FILE						*wd_tsfp;  			// Pointer to the time stamp output file 
...
#define	WDSYNC_EOF_RECEIVED			0x00000008				// This Worker_Thread received an EOF packet from the Source Side of an E2E Operation
	...
	struct xdd_e2e				*wd_e2ep;			// Pointer to the e2e struct when needed
	struct xdd_target_state		*wd_tgtstp;			// Pointer to the target state struct
	struct xdd_sgio				*wd_sgiop;			// SGIO Structure Pointer
};
