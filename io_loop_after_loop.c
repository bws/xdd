...
xdd_lockstep_after_io_loop(ptds_t *p) {
	...
	lockstep_t *lsp;	// Pointer to lockstep information
	lockstep_t *lsp2;	// Pointer to lockstep information


	if (p->lsp == NULL) 
		return(0); // In this case a return value of zero is goodness

	lsp = p->lsp;		// It is assume at this point that this pointer exists
...
if (xgp->global_options & GO_DEBUG_INIT) fprintf(stderr,"io_loop_after_loop: enter, p=0x%x\n",p);
...
if (xgp->global_options & GO_DEBUG_INIT) fprintf(stderr,"io_loop_after_loop: exit, p=0x%x\n",p);
